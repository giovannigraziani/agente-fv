...
function calcola(input) {
  ...
  const potenza = Math.min(
    (consumo / (irr * pr * coeff)) * 1000,
    (superficie / PANEL_M2) * PANEL_KWP
  );
  ...
  const mensile = MESI.map((m, i) => ({
    mese: m,
    produzione: Math.round(produzioneAnnua * MESI_COEFF[i] / totC),
    consumoMensile: Math.round(consumo / 12),
  }));
  const confronto = Array.from({ length: 6 }, (_, i) => {
    const a = (i + 1) * 5;
    return {
      anno: `A.${a}`,
      senzaFV: Math.round(consumo * COSTO_KWH * Math.pow(1.03, a)),
      conFV: Math.round((consumo - autoconsumo) * COSTO_KWH * Math.pow(1.03, a)),
    };
  });
  const donut = [
    { name: "Autoconsumo", value: autoconsumo },
    { name: "Immissione rete", value: immissione },
  ];
...
function Spinner() {
  ...
  useEffect(() => {
    const id = setInterval(() => setA(x => x + 12), 50);
...
function RotatingSun() {
  const [a, setA] = useState(0);
  useEffect(() => {
    const id = setInterval(() => setA(x => (x + 0.4) % 360), 30);
    return () => clearInterval(id);
  }, []);
...
function PulseDot({ color = "#86efac" }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
      {[0, 1, 2].map(i => (
        <span key={i} style={{ width: 4, height: 4, borderRadius: "50%", background: color, opacity: op, display: "inline-block" }} />
      ))}
    </span>
  );
}
...
function PopFlash({ children, color, ready }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: flash ? `0 0 14px ${color}99` : "none",
      borderRadius: 12,
    }}>
      {children}
    </div>
...
export default function App() {
  ...
  const [input, setInput] = useState({
    nomeCliente: "", consumo: 50000, zona: "Centro",
    tetto: "Falda Sud", superficie: 500, autoconsumoPerc: 65,
  });
  ...
  const avvia = async () => {
    ...
    const lm = {};
    Object.keys(prompts).forEach(k => lm[k] = true);
...
Dati: Consumo ${input.consumo.toLocaleString()} kWh/anno | Zona ${input.zona} | Potenza ${res.potenzaArr} kWp | Risparmio €${res.risparmioTotale.toLocaleString()}/anno | Costo €${res.costoImpianto.toLocaleString()} | Payback ${res.payback} anni | Copertura: ${Math.round(res.produzioneAnnua / input.consumo * 100)}%`;
    ...
    const allTasks = [
      { key: "__verdetto__", prompt: verdPrompt },
      ...Object.entries(prompts).map(([key, prompt]) => ({ key, prompt })),
    ];
    const BATCH = 2;
        ...
        await Promise.all(batch.map(async ({ key, prompt }) => {
          try {
            ...
            if (key === "__verdetto__") {
              ...
              setKpiTexts(p => ({ ...p, [key]: "Analisi non disponibile." }));
              setKpiLoading(p => ({ ...p, [key]: false }));
            }
          }
        }));
  ...
  const inp = {
    ...
    border: "1px solid #166534", background: "rgba(255,255,255,0.07)",
    color: "#e8f5e9", fontSize: 14, boxSizing: "border-box",
  };
  const sel = { ...inp, background: "#1a3a2a" };
  const card = {
    background: "rgba(255,255,255,0.05)", borderRadius: 14,
    padding: mobile ? "12px 10px" : "18px 16px", border: "1px solid #166534",
  };
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "linear-gradient(135deg,#0f2417 0%,#1a3a2a 50%,#0d1f15 100%)",
      ...
      <div style={{
        ...
        <div style={{
          ...
          borderRadius: 20, padding: "4px 10px", fontSize: 11, color: "#86efac", whiteSpace: "nowrap",
        }}>🤖 AI</div>
      </div>

      <div style={{
        width: "100%", maxWidth: 900, margin: "0 auto",
        padding: mobile ? "16px 12px" : "28px 16px", boxSizing: "border-box",
      }}>
        {/* STEP 1 */}
        {step === 1 && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {FASCE.map((fa, i) => (
                    <button key={i} onClick={() => { setFascia(i); set("consumo", fa.default); }}
                      style={{
                        flex: 1, padding: mobile ? "8px 4px" : "10px 8px", borderRadius: 10,
                        border: `2px solid ${fascia === i ? "#22c55e" : "#166534"}`,
                        background: fascia === i ? "rgba(34,197,94,0.15)" : "transparent",
                        color: fascia === i ? "#4ade80" : "#86efac",
                        fontWeight: 600, fontSize: mobile ? 12 : 13, cursor: "pointer", transition: "all .2s",
                      }}>
                      {fa.label}
                    </button>
                  ))}
              ...
              <div>
                ...
                <input type="range" min={f.min} max={f.max} step={f.step}
                  value={Math.min(Math.max(input.consumo, f.min), f.max)}
              ...
              <div>
                ...
                <input type="range" min={100} max={5000} step={50} value={input.superficie}
              ...
              {/* Autoconsumo */}
              <div style={{ gridColumn: "1/-1" }}>
                <label style={{ display: "block", marginBottom: 6, color: "#86efac", fontWeight: 600, fontSize: 12 }}>
                  🔄 Autoconsumo stimato — <span style={{ color: "#4ade80", fontWeight: 700 }}>{input.autoconsumoPerc}%</span>
                </label>
                <input type="range" min={30} max={90} step={5} value={input.autoconsumoPerc}
                  onChange={e => set("autoconsumoPerc", +e.target.value)}
            ...
            <button onClick={avvia}
              style={{
                marginTop: 22, width: "100%", padding: "15px", borderRadius: 14, border: "none",
                background: "linear-gradient(90deg,#16a34a,#22c55e)", color: "white",
                fontSize: 16, fontWeight: 700, cursor: "pointer",
                boxShadow: "0 4px 20px rgba(34,197,94,0.35)", transition: "transform .15s",
              }}
          ...
          <div>
            ...
            <FadeIn delay={100}>
                ...
                {KPI_DEFS.map(k => {
                  ...
                  return (
                    <PopFlash key={k.key} color={k.color} ready={!!kpiTexts[k.key]}>
                      <div
                        style={{
                          background: isSelected ? `${k.color}18` : "rgba(255,255,255,0.05)",
                          borderRadius: 12,
                          padding: mobile ? "10px 6px" : "14px 10px",
                          textAlign: "center",
                          border: `1px solid ${isSelected ? k.color : k.color + "33"}`,
                          cursor: kpiTexts[k.key] ? "pointer" : "default",
                          transition: "transform .2s, box-shadow .2s, background .2s",
                          boxShadow: isSelected ? `0 0 16px ${k.color}44` : "none",
                        }}
                        onMouseEnter={e => {
                          if (!isSelected && kpiTexts[k.key]) {
                            e.currentTarget.style.transform = "scale(1.05)";
                            e.currentTarget.style.boxShadow = `0 6px 18px ${k.color}44`;
                          }
                        }}
                        onMouseLeave={e => {
                          if (!isSelected) {
              ...
              {selectedKpi && (
                <div style={{
                  ...
                  border: `1px solid ${selDef?.color || "#22c55e"}55`,
                  borderRadius: 12, padding: "12px 14px",
                  display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12,
                }}>
                  <div style={{ fontSize: 20, flexShrink: 0 }}>{selDef?.icon}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 10, color: selDef?.color, textTransform: "uppercase", letterSpacing: .8, marginBottom: 5, fontWeight: 700 }}>
                      {selDef?.label} — analisi AI
                    </div>
                  ...
                  <button onClick={() => setSelectedKpi(null)}
                    style={{ background: "none", border: "none", color: "#86efac", fontSize: 16, cursor: "pointer", flexShrink: 0, padding: "0 2px" }}>
                    ✕
                  </button>
                </div>
              )}
            ...
            <FadeIn delay={180}>
              <div style={{
                ...
                border: `1px solid ${verdColor}44`,
                borderRadius: 14, padding: "12px 16px", marginBottom: 16,
              }}>
                <div style={{ fontSize: 10, color: "#86efac", textTransform: "uppercase", letterSpacing: .8, marginBottom: 8, fontWeight: 700 }}>
                  🤖 Valutazione commerciale
                </div>
                {verdLoading
                  ? <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#86efac", fontSize: 12 }}>
                      <PulseDot color="#86efac" /><span>elaborazione in corso…</span>
                    </div>
                  : verd
                    ? <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: verd.nota ? 8 : 0, flexWrap: "wrap" }}>
                          <span style={{
                            background: verdColor, color: "#0a1a0f", fontWeight: 800,
                        ...
                        {verd.nota && (
                          <div style={{ borderTop: `1px solid ${verdColor}22`, paddingTop: 8, marginTop: 4 }}>
                            <FormattedText text={verd.nota} />
                          </div>
                        )}
                      </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "3fr 2fr", gap: 14, marginBottom: 14 }}>
                ...
                <div style={card}>
                  ...
                  <ResponsiveContainer width="100%" height={mobile ? 160 : 200}>
                    <BarChart data={risultato.mensile} barCategoryGap="25%" barGap={2} margin={{ top: 2, right: 4, left: 0, bottom: 0 }}>
                      ...
                      <YAxis tick={{ fill: "#86efac", fontSize: mobile ? 8 : 10 }} axisLine={false} tickLine={false} width={mobile ? 36 : 46}
                        tickFormatter={v => v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v} />
                      <Tooltip contentStyle={{ background: "#1a3a2a", border: "1px solid #22c55e", borderRadius: 8, color: "#e8f5e9", fontSize: 11 }}
                        formatter={(v, n) => [`${v.toLocaleString()} kWh`, n === "produzione" ? "Produzione FV" : "Consumo"]} />
                ...
                <div style={{ ...card, display: "flex", flexDirection: "column" }}>
                  ...
                  <ResponsiveContainer width="100%" height={mobile ? 130 : 160}>
                    <PieChart>
                      <Pie data={risultato.donut} dataKey="value" cx="50%" cy="50%"
                        innerRadius={mobile ? 38 : 48} outerRadius={mobile ? 58 : 70}
                        paddingAngle={4} labelLine={false}>
              ...
              <div style={{ ...card, marginBottom: 8 }}>
                ...
                <ResponsiveContainer width="100%" height={mobile ? 150 : 190}>
                  <LineChart data={risultato.confronto} margin={{ top: 4, right: 12, left: 0, bottom: 0 }}>
                    ...
                    <YAxis tick={{ fill: "#86efac", fontSize: mobile ? 9 : 11 }} axisLine={false} tickLine={false}
                      width={mobile ? 42 : 52} tickFormatter={v => `€${(v / 1000).toFixed(0)}k`} />
                    <Tooltip contentStyle={{ background: "#1a3a2a", border: "1px solid #22c55e", borderRadius: 8, color: "#e8f5e9", fontSize: 11 }}
            ...
            <FadeIn delay={400}>
              <div style={{
                background: "rgba(22,101,52,0.3)", borderRadius: 10, padding: "10px 14px",
                border: "1px solid #166534", display: "flex", justifyContent: "space-between",
                alignItems: "center", fontSize: 11, color: "#86efac", marginBottom: 14,
              }}>
